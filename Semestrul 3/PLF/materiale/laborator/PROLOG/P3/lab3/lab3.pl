...
% permutari(l1...ln) =[], n = 0
%                     inserare(permutari(l2...ln), l1), altfel

% permutari(L:list, R:list)
% permutari(i, o)

permutari([], []).
permutari([H|T], R) :- permutari(T, RP),
                      inserare(RP, H, R).



% combinari(l1...ln, k) =[], k = 0
...
aranjamente(L, K, R) :- combinari(L, K, RC),
                        permutari(RC, R).

% allsolutions(L:list, N:number, R:list)
% allsolutions(i, i, o)

allsolutions(L, N, R) :- findall(RPartial, aranjamente(L, N, RPartial), R).
